...
type Manager struct {
	mu          sync.Mutex
	factory     *wirehoundStreamFactory
...
func NewManager(broadcaster Broadcaster) *Manager {
	...
	m.factory = &wirehoundStreamFactory{mgr: m}
...
// wirehoundStreamFactory creates streams for the TCP assembler.
type wirehoundStreamFactory struct {
	mgr *Manager
}

func (f *wirehoundStreamFactory) New(netFlow, tcpFlow gopacket.Flow) tcpassembly.Stream {
...
func (s *wirehoundStream) readLoop() {
	buf := make([]byte, 4096)
	for {
