...
func handleUpload(eng *engine.Engine) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		tmpDir := os.TempDir()
		tmpFile, err := os.CreateTemp(tmpDir, "wirehound-*.pcap")
		if err != nil {
...
func handleExport(eng *engine.Engine) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=\"wirehound-%s.pcap\"", time.Now().Format("20060102-150405")))
